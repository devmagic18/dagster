---
title: Jobs | Dagster
description: A job is an executable particularization of a graph that binds it to resources and configuration.
---

# Jobs

A job is an executable particularization of a graph that binds it to resources and configuration.
...
Dagster is built around the observation that any data DAG typically contains a logical core of transformation (the graph), which is reusable across a set of environments or scenarios. The graph needs to be "particularized" for each environment, usually by plugging in configuration and services that are specific to that environment.
...
[Schedules](/concepts/partitions-schedules-sensors/schedules) and [sensors](/concepts/partitions-schedules-sensors/sensors) target jobs.  You can also launch jobs manually from Dagit, GraphQL APIs, or the command line.

## Automatic coercion of graphs to jobs

The simplest way to create a job is to do nothing at all.  I.e. you can include a graph in a repository, or target it from a schedule or sensor, and Dagster will automatically coerce it into a job.
...
def do_something(context):
    context.log.info(f"number: " + context.op_config["number"])
    context.log.info(f"my_resource: " + context.resources.my_resource)
